         ...
         <div class="d-flex justify-content-center div2">
            ...
            <table class="table table-dark">
                <thead>
                    ...
                    <td>Name</td>
                    <td>Mobile</td>
                    <td>Guest's Faculty</td>
                    <td>Guest's Department</td>
                    <td>Position</td>
                    <td>Action</td>

                </thead>
                <tbody>
                    @foreach ($cusdata7 as $cdb)
                <tr>
                    <td>{{$cdb->id}}</td>
                    <td><img src="{{asset('/uploads/guests/'.$cdb->photo) }}" alt="image" width="50px" height="50px"></td>
                    <td>{{$cdb->username}}</td>
                    <td>{{$cdb->guest_name}}</td>
                    <td>{{$cdb->mobile}}</td>
                    <td>{{$cdb->guest_faculty}}</td>
                    <td>{{$cdb->guest_department}}</td>
                    <td>{{$cdb->guest_position}}</td>
                    <td>
                        <div class="dropdown">
        ...
        @if (sizeof($cusdata7)!==0)
                ...
                <div class="modal fade" id="staticBackdrop{{$i}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel1" aria-hidden="true">
                    ...
                    <div class="modal-content">
                        ...
                        <button type="button" class="btn btn-primary"><a class="dropdown-item" href="{{url('updateguestpassword-guest/'.$i)}}" data-bs-target="#staticBackdrop1">Yes</a></button>
                        </div>
                    </div>
                    </div>
                </div>
